 ...
 * own addon, rather than using anything from Rebar. We recommend checking
...
class CommandBuilder(val command: ArgumentBuilder<CommandSourceStack, *>) {
    ...
    fun permission(permission: String) = requires(Component.translatable("rebar.message.command.error.no_permission")) { it.hasPermission(permission) }
    ...
    inline fun executesWithPlayer(crossinline handler: CommandContext<CommandSourceStack>.(Player) -> Unit) {
        requires(Component.translatable("rebar.message.command.error.must_be_player")) { it is Player }
        executes {
