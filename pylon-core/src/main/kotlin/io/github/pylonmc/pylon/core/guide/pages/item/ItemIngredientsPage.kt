...
open class ItemIngredientsPage(val input: FluidOrItem) : TabbedGuidePage {
    ...
    private val ingredientsTab = ItemListDisplayTab(calculation.inputs.map(::fluidOrItemButton))
    ...
    private val byproductsTab = ItemListDisplayTab(calculation.byproducts.map(::fluidOrItemButton))
...
private fun fluidOrItemButton(fluidOrItem: FluidOrItem) = when (fluidOrItem) {
    ...
    is FluidOrItem.Item -> ItemButton(fluidOrItem.item)
}