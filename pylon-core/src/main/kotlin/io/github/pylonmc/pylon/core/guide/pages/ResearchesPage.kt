...
class ResearchesPage internal constructor() : SimpleDynamicGuidePage(
    pylonKey("researches"),
    Material.BREWING_STAND,
    { PylonRegistry.RESEARCHES.getKeys().map { ResearchButton(it) }.toMutableList() }
) {

    override fun getHeader(player: Player) = super.getHeader(player)
        .addIngredient('s', PageButton(PylonGuide.searchResearchesPage))
}